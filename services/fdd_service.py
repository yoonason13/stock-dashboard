...
항목이 없는 경우 "미확인"으로 표기하세요."""
    ...
    return _call_claude(system, prompt, max_tokens=1500, use_thinking=False)
...
각 항목마다 수치와 함께 **긍정(✅)/중립(⚠️)/부정(🚨)** 신호를 표시하세요."""
    ...
    return _call_claude(system, prompt, max_tokens=1500, use_thinking=True)
...
숫자와 근거를 반드시 포함하고, 전문적이고 객관적인 어조로 작성하세요."""
    ...
    return _call_claude(system, prompt, max_tokens=2500, use_thinking=True)
